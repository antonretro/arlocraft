...
export class ZombieModel extends BaseModel {
  constructor() {
    super();
    ...
    this.armR = this.createPart('armR', 4 * unit, 12 * unit, 4 * unit, 6 * unit, 18 * unit, 0, armUV);
    this.armL = this.createPart('armL', 4 * unit, 12 * unit, 4 * unit, -6 * unit, 18 * unit, 0, armUV);

    // Initial pose for Zombie: Arms extended forward
  ...
  animateWalking(t) {
    ...
    // Zombie arms: gentle oscillating bob while forward
    const armBob = Math.sin(t * 0.5) * 0.05;
    this.parts.armR.rotation.x = -Math.PI / 2 + armBob;
    this.parts.armL.rotation.x = -Math.PI / 2 - armBob;

    if (this.parts.headGroup) {
      this.parts.headGroup.rotation.y = Math.sin(t * 0.3) * 0.2;
    }
  }
}
