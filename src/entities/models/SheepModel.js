...
export class SheepModel extends BaseModel {
  constructor() {
    ...
    this.body = this.createPart('body', 10 * unit, 10 * unit, 16 * unit, 0, 2 * unit, 0, {
      u: 28, v: 8, w: 10, h: 10, d: 16
    });
    ...
    this.legFR = this.createPart('legFR', 4 * unit, 12 * unit, 4 * unit, 3 * unit, -10 * unit, 5 * unit, legUV);
    this.legFL = this.createPart('legFL', 4 * unit, 12 * unit, 4 * unit, -3 * unit, -10 * unit, 5 * unit, legUV);
    this.legBR = this.createPart('legBR', 4 * unit, 12 * unit, 4 * unit, 3 * unit, -10 * unit, -5 * unit, legUV);
    this.legBL = this.createPart('legBL', 4 * unit, 12 * unit, 4 * unit, -3 * unit, -10 * unit, -5 * unit, legUV);
