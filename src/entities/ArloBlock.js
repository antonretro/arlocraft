...
/**
 * ArloBlock: The ultimate anti-virus ally.
 * Can be commanded, petted, and fed to grow stronger.
 */
export class ArloBlock extends MobEntity {
  ...
  feed(foodItem) {
    ...
    // Eating jump
    this.velocity.y = 2.5;
    console.log(`Arlo ate ${foodItem.name}!`);
  }
  ...
  update(delta) {
    ...
    if (this.state === 'FOLLOW') {
      if (distToPlayer > 18) {
        ...
        const dir = new THREE.Vector3()
          .subVectors(cam.position, this.mesh.position)
          .normalize();
    ...
    // Snap to terrain
    const floorY = this.game.world.getTerrainHeight(
