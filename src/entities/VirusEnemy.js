...
export class VirusEnemy {
    ...
    update(delta) {
        ...
        const groundY = this.game.world.getTerrainHeight(this.mesh.position.x, this.mesh.position.z) + halfHeight;
        const waterY = this.game.world.getWaterSurfaceYAt(this.mesh.position.x, this.mesh.position.z);
        const targetY = waterY === null ? groundY : Math.max(groundY, waterY + (halfHeight * 0.2));
        this.mesh.position.y = THREE.MathUtils.lerp(this.mesh.position.y, targetY, Math.min(1, delta * 7));
        ...
        if (dist < 1.4 && this.hitCooldown <= 0) {
            ...
            this.game.physics.applyKnockback(knock, this.config.boss ? 3.1 : 2.2, this.config.boss ? 0.42 : 0.28);
    ...
    die() {
        ...
        window.dispatchEvent(new CustomEvent('enemy-defeated', { detail: { id: this.config.id, xp: this.config.xp } }));
    }
}
