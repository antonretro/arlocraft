...
export class BirdEntity extends MobEntity {
    ...
    update(delta) {
        ...
        const targetX = this.game.camera.instance.position.x + Math.cos(angle) * radius;
        const targetZ = this.game.camera.instance.position.z + Math.sin(angle) * radius;
        
        this.mesh.position.x = THREE.MathUtils.lerp(this.mesh.position.x, targetX, delta * 2);
        this.mesh.position.z = THREE.MathUtils.lerp(this.mesh.position.z, targetZ, delta * 2);
