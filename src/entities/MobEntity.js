...
export class MobEntity {
    ...
    showChat() {
        ...
        const line = this.config.chat[Math.floor(Math.random() * this.config.chat.length)];
        window.dispatchEvent(new CustomEvent('action-prompt', { detail: { type: line } }));
    ...
    takeDamage(amount, sourcePosition = null, knockback = 0.7) {
        ...
        if (sourcePosition) {
            const away = this.tmpKnockbackDir.subVectors(this.mesh.position, sourcePosition);
            away.y = 0;
        ...
        if (this.hp <= 0) {
            ...
            window.dispatchEvent(new CustomEvent('mob-death', { detail: { id: this.config.id } }));
        }
    }

    update(delta) {
        if (this.dead) return;
        if (this.contactCooldown > 0) this.contactCooldown = Math.max(0, this.contactCooldown - delta);
        ...
        const canUseWater = Boolean(this.config.aquatic || this.config.canSwim || isHostile);
        ...
        if (this.state === 'CHASE' && isHostile) {
            ...
            this.moveVelocity.lerp(dir.multiplyScalar(chaseSpeed), Math.min(1, delta * 7));
            this.moveWithCollision(this.moveVelocity.x * delta, this.moveVelocity.z * delta);
            ...
            const dir = this.tmpMoveDir.subVectors(this.targetPos, this.mesh.position);
            dir.y = 0;
            if (dir.lengthSq() > 0.0001) dir.normalize();
            const wanderSpeed = this.maxMoveSpeed * 0.22;
            this.moveVelocity.lerp(dir.multiplyScalar(wanderSpeed), Math.min(1, delta * 6));
            this.moveWithCollision(this.moveVelocity.x * delta, this.moveVelocity.z * delta);
        } else {
    ...
    updateSurfaceY(delta, canUseWater) {
        ...
        const halfHeight = (this.geometry.parameters.height * this.mesh.scale.y) * 0.5;
        const groundY = world.getTerrainHeight(this.mesh.position.x, this.mesh.position.z) + halfHeight;
        const waterY = world.getWaterSurfaceYAt(this.mesh.position.x, this.mesh.position.z);
        ...
        this.mesh.position.y = THREE.MathUtils.lerp(this.mesh.position.y, targetY, Math.min(1, delta * 8));
    }

    checkMergeEvolution() {
        const others = this.game.entities.entities.filter(e => e !== this && !e.dead && e.config.id === this.config.id);
    ...
    evolve(other) {
        ...
        setTimeout(() => { if (this.material) this.material.emissiveIntensity = 0; }, 200);
