export const ocean_ruins = {
    ...
    blueprints: (x, y, z) => {
        ...
        const wallBlocks = ['mossy_cobblestone', 'cobblestone', 'stone_bricks', 'mossy_stone_bricks', 'cracked_stone_bricks'];
        const wallPositions = [
            // North wall
        ...
        for (const { dx, dz } of wallPositions) {
            ...
            for (let dy = 1; dy <= maxH; dy++) {
                const id = wallBlocks[Math.abs((dx * 3 + dz * 7 + dy) % wallBlocks.length)];
                blocks.push({ x: x + dx, y: y + dy, z: z + dz, id });
            }
        }

        // Collapsed pillars
        for (const [px, pz] of [[-3, -3], [3, -3], [-3, 3]]) {
            for (let dy = 1; dy <= 5; dy++) {
                blocks.push({ x: x + px, y: y + dy, z: z + pz, id: 'stone_bricks' });
            }
            // Fallen pillar pieces
            blocks.push({ x: x + px + 1, y: y + 1, z: z + pz, id: 'cracked_stone_bricks' });
            blocks.push({ x: x + px + 2, y: y + 1, z: z + pz, id: 'mossy_stone_bricks' });
        }
        ...
        const coralTypes = ['tube_coral_block', 'brain_coral_block', 'bubble_coral_block', 'horn_coral_block'];
        for (let dx = -3; dx <= 3; dx += 2) {
            for (let dz = -3; dz <= 3; dz += 2) {
                if (Math.abs(dx) === 3 && Math.abs(dz) === 3) continue;
                blocks.push({ x: x + dx, y: y + 1, z: z + dz, id: coralTypes[Math.abs((dx + dz) % coralTypes.length)] });
            }
        }
