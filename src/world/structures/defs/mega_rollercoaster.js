export const mega_rollercoaster = {
    ...
    blueprints: (x, y, z) => {
        ...
        for (const [sx, sz] of supports) {
            ...
            blocks.push({ x: x + sx, y: baseY + height + 1, z: z + sz, id: 'sea_lantern' });
        }
        ...
        for (let i = -12; i <= 12; i++) {
            ...
            // North track
            blocks.push({ x: x + i,  y: baseY + 10 + waveN,     z: z - 12, id: 'oak_planks' });
            blocks.push({ x: x + i,  y: baseY + 10 + waveN + 1, z: z - 12, id: 'powered_rail' });

            // East track
            blocks.push({ x: x + 12, y: baseY + 10 - waveE,     z: z + i, id: 'oak_planks' });
            blocks.push({ x: x + 12, y: baseY + 10 - waveE + 1, z: z + i, id: 'rail' });

            // South track
            blocks.push({ x: x - i,  y: baseY + 8 + waveS,     z: z + 12, id: 'oak_planks' });
            blocks.push({ x: x - i,  y: baseY + 8 + waveS + 1, z: z + 12, id: 'powered_rail' });

            // West track
            blocks.push({ x: x - 12, y: baseY + 8 - waveW,     z: z - i, id: 'oak_planks' });
            blocks.push({ x: x - 12, y: baseY + 8 - waveW + 1, z: z - i, id: 'rail' });
        }

        // Diagonal cross-supports (X bracing) between corner columns
        const braces = [
            [[-12, -12], [0, -12]], [[0, -12], [12, -12]],
            [[12, -12], [12, 0]],   [[12, 0],  [12, 12]],
            [[12, 12],  [0, 12]],   [[0, 12],  [-12, 12]],
            [[-12, 12], [-12, 0]],  [[-12, 0], [-12, -12]],
        ];
        ...
        const flagColors = ['wool_red', 'wool_yellow', 'wool_blue', 'wool_green', 'wool_orange', 'wool_purple'];
        for (let t = -10; t <= 10; t += 4) {
            const fi = Math.abs(t) % flagColors.length;
            const waveN2 = Math.floor(3 + (Math.sin(t * 0.45) * 3));
            blocks.push({ x: x + t, y: baseY + 12 + waveN2, z: z - 12, id: flagColors[fi] });
            blocks.push({ x: x + 12, y: baseY + 12 - waveN2, z: z + t, id: flagColors[(fi + 2) % flagColors.length] });
        }
