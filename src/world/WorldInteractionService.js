...
export class WorldInteractionService {
    ...
    breakBlockAt(x, y, z) {
        ...
        if (pairState) {
            ...
            this.world.mutations.removeBlockByKey(pairState.pairKey, { skipChangeTracking: true });
            this.world.explosions.spawnBreakParticles(x, pairState.pairY, z, pairState.pairId);
        }
        ...
        // Gravity collapse
        if (this.world.blocks.isGravityBlock(id) || ['stone', 'dirt', 'grass_block', 'sand'].includes(id)) {
    ...
    _applyGravityAbove(x, y, z) {
        for (let dy = 1; dy <= 28; dy++) {
            const ay = y + dy;
            const aboveId = this.world.state.blockMap.get(this.world.coords.getKey(x, ay, z));
            if (!aboveId) break;
    ...
    mineBlockProgress(x, y, z, id, selectedItem, mode, delta) {
        ...
        if (this.world.state.miningState.key !== key) {
            ...
            this.world.state.miningState.progress = 0;
            this.world.state.miningState.required = this.world.blocks.computeMineDuration(id, selectedItem, mode);
        }
