...
export class BlockRulesService {
  constructor(world) {
    ...
    this.blockXpById = new Map(
      BLOCKS.map((block) => [block.id, block.xp ?? 0])
    );
    this.blockHardnessById = new Map(
      BLOCKS.map((block) => [block.id, block.hardness ?? 1])
    );
  ...
  _initPairs() {
    for (const block of BLOCKS) {
      ...
      if (!this.reversePairById.has(pairId)) {
        this.reversePairById.set(pairId, {
          pairId: block.id,
          pairOffsetY: -pairOffsetY,
        });
      }
    }
  }
  ...
  computeMineDuration(blockId, selectedItem, mode) {
    ...
    const hardness = Math.max(
      0.15,
      Number(this.blockHardnessById.get(normalizeBlockVariantId(blockId)) ?? 1)
    );
    let efficiency = 1;
    if (selectedItem?.id) {
  ...
  getBlockPickId(id) {
    ...
    if (id === 'beetroot' || id?.startsWith('beetroot_stage'))
      return 'beetroot';
