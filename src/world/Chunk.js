...
const EXCLUDED_FROM_RANDOM_POOL = new Set([
  'dungeon_chamber',
  'village_manor',
  'village_hut',
]);
...
export class Chunk {
  ...
  noteTouchedGenerationChunk(x, y, z) {
    ...
    const loaded = this.world.chunkManager.getChunk(
      target.cx,
      target.cy,
      target.cz
    );
    if (loaded && !loaded.destroyed) {
      ...
      Math.abs(target.cx - this.cx) <= 1 &&
      Math.abs(target.cy - this.cy) <= 1 &&
      Math.abs(target.cz - this.cz) <= 1
    ) {
  ...
  finalizeGenerationPass() {
    for (const target of this.touchedGenerationChunks.values()) {
      const chunk = this.world.chunkManager.getChunk(
        target.cx,
        target.cy,
        target.cz
      );
  ...
  selectUndergroundBlock(wx, y, wz, biome) {
    ...
    // Redstone (y < -35)
    if (y < -35 && h1 < 0.0045)
      return isDeep ? 'deepslate_redstone_ore' : 'redstone_ore';
    // Gold     (y -50..−10)
    if (y > -50 && y < -10 && h1 < 0.006)
      return isDeep ? 'deepslate_gold_ore' : 'gold';
    // Lapis    (y -40..+5)
    if (y > -40 && y < 5 && h2 < 0.004)
      return isDeep ? 'deepslate_lapis_ore' : 'lapis_ore';
    // Iron     (y -30..+25)
    if (y > -30 && y < 25 && h1 < 0.014)
      return isDeep ? 'deepslate_iron_ore' : 'iron';
    // Copper   (y -16..+48)
    if (y > -16 && y < 48 && h3 < 0.009)
      return isDeep ? 'deepslate_copper_ore' : 'copper';
    // Coal     (y > -15)
    if (y > -15 && h2 < 0.024) return 'coal';
    // Emerald  (highlands only, y > -20)
    if (biome?.id === 'highlands' && y > -20 && h3 < 0.001)
      return 'emerald_ore';
  ...
  placeUnderwaterStructure(centerX, centerZ) {
    ...
    const pick =
      pool[
        Math.floor(this.world.hash2D(this.cx - 77, this.cz + 33) * pool.length)
      ];
  ...
  placeRandomStructure(x, y, z, biome) {
    ...
    if (!this.world.corruptionEnabled) {
      ...
      if (virusRoll < 0.94) {
        const civicOnly = CIVIC_STRUCTURE_POOL.filter((key) =>
          keys.includes(key)
        );
    ...
    if (Array.isArray(struct.biomes) && struct.biomes.length > 0) {
      if (!struct.biomes.includes('any') && !struct.biomes.includes(biome.id))
        return;
    } else {
    ...
    if (
      structKey === 'mega_rollercoaster' &&
      this.world.hash2D(x + 311, z - 125) < 0.72
    )
      return;
    if (
      structKey === 'spinning_ride' &&
      this.world.hash2D(x - 207, z + 519) < 0.5
    )
      return;
  ...
  chooseTreeType(x, z, biome) {
    ...
    if (biome.id === 'badlands' || biome.id === 'canyon')
      return hash > 0.5 ? 'acacia' : 'palm';
    if (biome.id === 'forest')
      return hash > 0.65
        ? 'birch'
        : hash > 0.35
    ...
    if (biome.id === 'plains')
      return hash > 0.8 ? 'cherry' : hash > 0.4 ? 'oak' : 'birch';
    if (biome.id === 'meadow')
      return hash > 0.75 ? 'cherry' : hash > 0.5 ? 'birch' : 'oak';
  ...
  placePalmCanopy(x, y, z, radius, leafId) {
    ...
    for (const [dx, dz] of directions) {
      const length = Math.max(
        2,
        radius + (Math.abs(dx) + Math.abs(dz) > 1 ? -1 : 0)
      );
  ...
  placeTree(x, y, z, biome) {
    ...
    const trunkHeight =
      config.height + Math.floor(this.world.hash2D(x + 7, z - 19) * 2);
    ...
    if (config.style === 'spire') {
      for (let i = 1; i <= 2; i++)
        this.addGeneratedBlock(x, topY + i, z, config.trunk);
  ...
  async generate() {
    ...
    try {
      ...
      if (this.world.chunkGenerator && this.world.chunkGenerator.available) {
        try {
          const results = await this.world.chunkGenerator.generateChunk(
            this.cx,
            this.cy,
            this.cz
          );
          ...
          console.warn(
            '[ArloCraft] Chunk Worker failed, falling back to sync',
            error
          );
        }
      }
  ...
  placeSettlementCluster(centerX, centerY, centerZ) {
    ...
    if (type === 'desert_town') {
      ...
      structures = [
        STRUCTURES.castle,
        STRUCTURES.castle_wall,
        STRUCTURES.castle_wall,
        STRUCTURES.blacksmith_forge,
      ];
      offsets = [
        [0, 0],
        [0, 8],
      ...
      structures = [
        STRUCTURES.village_manor,
        STRUCTURES.village_manor,
        STRUCTURES.village_hut,
        STRUCTURES.village_well,
      ];
    } else {
      structures = [
        STRUCTURES.village_hut,
