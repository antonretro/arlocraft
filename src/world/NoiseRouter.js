...
export class NoiseRouter {
  ...
  getClimate(x, z) {
    ...
    const continental = clamp01(
      this.sample2D(this.depthNoise, x + sx, z + sz, 620) * 0.5 + 0.5
    );
    const erosion = clamp01(
      this.sample2D(this.scaleNoise, x - 4400 + sx, z + 2800 + sz, 340) * 0.5 +
        0.5
    );
    const ridge = clamp01(
      this.sample2D(this.ridgeNoise, x + 1200 + sx, z - 1900 + sz, 240) * 0.5 +
        0.5
    );

    const rawTemp = clamp01(
      this.sample2D(this.tempNoise, x + 9000 + sx, z - 3000 + sz, 560) * 0.5 +
        0.5
    );
    const rawHumidity = clamp01(
      this.sample2D(this.humidNoise, x - 6000 + sx, z + 7000 + sz, 520) * 0.5 +
        0.5
    );

    const elevation = clamp01(
      continental * 0.58 + (1 - erosion) * 0.19 + ridge * 0.23
    );
    const distFromOrigin = Math.sqrt(x * x + z * z);
    const warmBias = Math.max(0, 1 - distFromOrigin / 96) * 0.35;
    const temperature = clamp01(
      rawTemp + warmBias + (continental - 0.5) * 0.08 - (elevation - 0.5) * 0.13
    );
    const humidity = clamp01(
      rawHumidity + (0.5 - temperature) * 0.12 - (continental - 0.5) * 0.06
    );
  ...
  selectBiomeID(temperature, humidity, elevation, ridge) {
    ...
    if (elevation > 0.68 && humidity >= 0.38 && temperature > 0.28)
      return 'highlands';

    if (humidity > 0.78 && temperature > 0.32) return 'swamp';
    if (humidity > 0.62 && temperature > 0.34 && elevation < 0.66)
      return 'forest';
  ...
  getTerrainHeight(x, z) {
    ...
    const ridgeRaw = Math.abs(
      this.sample2D(this.ridgeNoise, x - 1300, z + 900, 210)
    );
    const ridge = ridgeRaw * ridgeRaw;

    const macroVariation = macro * (18 * blended.mcScale);
    const detailVariation =
      detail * (6 * blended.detailStrength * Math.max(0.25, blended.mcScale));
  ...
  getDensity(x, y, z) {
    ...
    const caveThreshold =
      -0.14 + Math.min(0.12, Math.max(-0.06, (y - 24) * 0.002));
    return (
      vertical + (caveSignal - caveThreshold) * this.getSurface(x, z) * 3.6
    );
  }
}
