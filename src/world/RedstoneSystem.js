...
export class RedstoneSystem {
  ...
  calculatePower(x, y, z) {
    ...
    for (const [nx, ny, nz] of neighbors) {
      ...
      const nPower =
        this.powerMap.get(this.world.coords.getKey(nx, ny, nz)) || 0;
  ...
  onPowerChanged(x, y, z, power, id) {
    ...
    if (id === 'redstone_lamp') {
      if (power > 0) {
        this.world.mutations.setBlock(x, y, z, 'redstone_lamp:on', null, {
          silent: true,
        });
      } else {
        this.world.mutations.setBlock(x, y, z, 'redstone_lamp', null, {
          silent: true,
        });
      }
    }
