...
export const BIOME_GROUND_LIFE = {
    ...
    meadow:    ['short_grass', 'tall_grass_bottom', 'dandelion', 'poppy', 'orange_tulip', 'red_tulip', 'pink_tulip', 'white_tulip', 'azure_bluet', 'oxeye_daisy', 'cornflower', 'allium', 'blueberry', 'lilac', 'peony', 'rose_bush', 'wheat', 'beetroot'],
...
const STAGED_CROPS = ['blueberry', 'strawberry', 'tomato', 'potato', 'carrot', 'wheat', 'beetroot'];
...
function chooseTreeType(x, z, biome, corruptionEnabled, seed) {
    ...
    if (biome.id === 'forest')  return h > 0.6 ? 'birch' : (h > 0.3 ? 'oak' : 'pine');
    if (biome.id === 'swamp')   return 'willow';
    if (biome.id === 'plains' || biome.id === 'meadow') return h > 0.8 ? 'cherry' : 'oak';
...
export function addTree(planMap, changedMap, x, y, z, biome, corruptionEnabled, noise, seed, setPlannedBlock) {
    const treeType    = chooseTreeType(x, z, biome, corruptionEnabled, seed);
    const config      = TREE_CONFIGS[treeType] ?? TREE_CONFIGS.oak;
    const trunkHeight = config.height + Math.floor(hash2D(x + 7, z - 19, seed) * 2);
...
//   (planMap, changedMap, x, y, z, blockId) → void
export function addGroundLife(planMap, changedMap, x, y, z, biome, noise, seed, setPlannedPlant, surfaceId) {
