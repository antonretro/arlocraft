...
export class WorldTerrainService {
  ...
  getTerrainHeight(x, z) {
    ...
    const clamped = getRawTerrainHeight(
      this.router,
      this.noise,
      gx,
      gz,
  ...
  getBlendedColor(x, z, property = 'color') {
    ...
    if (this._blendedColorCache.has(cacheKey))
      return this._blendedColorCache.get(cacheKey);
  ...
  isPositionInWater(x, y, z) {
    ...
    const key = this.world.coords.getKey(
      Math.round(x),
      Math.round(y),
      Math.round(z)
    );
  ...
  hash3D(x, y, z) {
    const v =
      Math.sin(x * 12.9898 + y * 78.233 + z * 37.719 + this.seed) * 43758.5453;
    return v - Math.floor(v);
  }
  ...
  valueNoise2D(x, z, scale) {
    ...
    const x0 = Math.floor(sx),
      z0 = Math.floor(sz);
    const tx = this._smoothstep(sx - x0),
      tz = this._smoothstep(sz - z0);
    return this._lerp(
  ...
  isPathAt(x, z) {
    const trunk = Math.abs(
      this.noise.simplex2D(x * 0.02 + 1307, z * 0.02 - 811)
    );
    const branch = Math.abs(
      this.noise.simplex2D(x * 0.04 - 547, z * 0.04 + 199)
    );
  ...
  shouldCarveCave(x, y, z, terrainHeight) {
    ...
    const nB =
      this.noise.simplex3D(x * 0.01 + 17, y * 0.015 - 9, z * 0.01 + 4) * 0.5 +
      0.5;
    const nC =
      this.noise.simplex3D(x * 0.005 - 41, y * 0.05 + 12, z * 0.005 + 63) *
        0.5 +
      0.5;
    ...
    const fissureNoise =
      this.noise.simplex3D(x * 0.002 + 9, y * 0.02 - 31, z * 0.002 - 21) * 0.5 +
      0.5;
    const fissure =
      Math.abs(fissureNoise - 0.5) < 0.03 && y < terrainHeight - 8;
  ...
  isStructureChunkAnchor(cx, cz, radius = 2) {
    ...
    for (let dx = -radius; dx <= radius; dx++) {
      for (let dz = -radius; dz <= radius; dz++) {
        ...
        if (this.getStructureChunkScore(cx + dx, cz + dz) >= score)
          return false;
      }
    }
    return true;
