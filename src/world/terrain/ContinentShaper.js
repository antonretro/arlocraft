...
export function getContinentMask(noise, x, z, seed) {
    ...
    const base = (noise.simplex2D(
        (x * 0.00055) + seed * 0.0001,
        (z * 0.00055) - seed * 0.00007
    ) * 0.5) + 0.5;

    // Medium-scale variation (≈400 blocks) — bays and peninsulas
    const detail = (noise.simplex2D(
...
export function getRawTerrainHeight(router, noise, x, z, seed) {
    ...
    if (continent < 0.35) {
        ...
        const mountBase = lerp(HILLS_MAX_OFFSET, MOUNTAIN_MAX_OFFSET, smoothstep(t));
...
export function getColumnHeight(router, noise, x, z, seed) {
    ...
    if (dist < SPAWN_BLEND_RADIUS) {
        const t = (dist - SPAWN_FLAT_RADIUS) / (SPAWN_BLEND_RADIUS - SPAWN_FLAT_RADIUS);
