...
export class Physics {
  ...
  resolveSafeSpawn(x, y, z, maxRadius = 24) {
    ...
    const needsRelocate = this.world.isPositionInWater(
      safeX,
      candidateY,
      safeZ
    );
    const localClearY = !needsRelocate
      ? findClearY(safeX, safeZ, candidateY)
      : null;
    if (localClearY !== null) {
  ...
  update(delta, input, lookYaw) {
    ...
    const waterSurfaceY = inWater
      ? this.world.getWaterSurfaceYAt(this.position.x, this.position.z)
      : null;
    const nearWaterSurface =
      waterSurfaceY !== null &&
      waterSurfaceY - this.position.y <= this.waterSurfaceExitWindow;
    const grounded = !inWater && this.mode === 'SURVIVAL' && this.isGrounded();

    let wantsToCrouch =
      this.mode === 'SURVIVAL' && (keys['ShiftLeft'] || keys['ShiftRight']);
    ...
    const blockUnderFeet = this.world.getBlockAt(
      this.position.x,
      this.position.y - 0.1,
      this.position.z
    );
    ...
    for (let i = 0; i < steps; i++) {
      ...
      const nearWaterSurface =
        ...
        waterSurfaceY - this.position.y <= this.waterSurfaceExitWindow;
      const grounded =
        !inWater && this.mode === 'SURVIVAL' && this.isGrounded();
      ...
      if (this.mode === 'SURVIVAL') {
        if (
          !this.canOccupyAt(this.position.x, this.position.y, this.position.z)
        ) {
      ...
      if (this.mode === 'SURVIVAL') {
        if (
          ...
          const groundNext = this.getGroundYAt(
            this.position.x,
            oldZ,
            this.position.y
          );
          ...
          const maxStepHeight =
            inWater && nearWaterSurface
              ? this.waterExitStepHeight
              : this.maxStepHeight;

          const canStep =
            this.velocity.y <= 0.2 &&
            (grounded || (inWater && nearWaterSurface));
          if (
            canStep &&
            stepUp > 0 &&
            stepUp <= maxStepHeight &&
            this.canOccupyAt(this.position.x, stepCandidateY, oldZ)
          ) {
      ...
      if (this.mode === 'SURVIVAL') {
        if (
          ...
          const groundCurrent = this.getGroundYAt(
            this.position.x,
            preStepZ,
            oldY
          );
          ...
          if (
            ...
            stepUp > 0 &&
            stepUp <= maxStepHeight &&
            this.canOccupyAt(this.position.x, stepCandidateY, this.position.z)
          ) {
            this.position.y = stepCandidateY;
          } else {
      ...
      if (this.mode === 'SURVIVAL' && !inWater) {
        const floorY = this.getGroundYAt(
          this.position.x,
          this.position.z,
          this.position.y
        );
