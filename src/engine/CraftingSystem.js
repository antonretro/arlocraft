...
export class CraftingSystem {
    ...
    match(craftingGrid) {
        const ids = gridToIds(craftingGrid);
        ...
        for (const recipe of this.recipes) {
            ...
            const matchedNormal = this.matchesShaped(normalizedGrid, recipe.normalizedPattern, recipe.key);
            const matchedMirror = !matchedNormal && this.matchesShaped(normalizedGrid, recipe.mirroredPattern, recipe.key);
    ...
    matchesShaped(normalizedGrid, patternRows, keyMap) {
        ...
        if ((normalizedGrid[0]?.length ?? 0) !== (patternRows[0]?.length ?? 0)) return false;
