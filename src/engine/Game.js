...
export class Game {
  constructor() {
    ...
    this.selectedStartMode =
      this.settings.preferredMode === 'CREATIVE' ? 'CREATIVE' : 'SURVIVAL';
  ...
  applySaveData(data) {
    ...
    if (Array.isArray(data.inventory)) {
      ...
      while (this.gameState.inventory.length < 36)
        this.gameState.inventory.push(null);
    }
    this.gameState.offhand = migrateInventoryItem(data?.offhand ?? null);
    if (Array.isArray(data.craftingGrid)) {
      this.gameState.craftingGrid = data.craftingGrid
        .slice(0, 9)
        .map(migrateInventoryItem);
      while (this.gameState.craftingGrid.length < 9)
        this.gameState.craftingGrid.push(null);
    }
    ...
    if (data.player) {
      ...
      if (Number.isFinite(data.player.hunger))
        this.gameState.hunger = data.player.hunger;
      if (data.player.look) {
    ...
    if (data.stats) {
      if (Number.isFinite(data.stats.level))
        this.stats.level = data.stats.level;
      if (Number.isFinite(data.stats.xp)) this.stats.xp = data.stats.xp;
      if (Number.isFinite(data.stats.xpToNextLevel))
        this.stats.xpToNextLevel = data.stats.xpToNextLevel;
    ...
    window.dispatchEvent(new CustomEvent('inventory-changed'));
    window.dispatchEvent(
      new CustomEvent('offhand-changed', { detail: this.gameState.offhand })
    );
    window.dispatchEvent(
      new CustomEvent('hp-changed', { detail: this.gameState.hp })
    );
    ...
    window.dispatchEvent(
      new CustomEvent('xp-changed', {
        detail: { xp: this.stats.xp, max: this.stats.xpToNextLevel },
      })
    );
  }
  ...
  updateCrosshairVisibility() {
    ...
    const shouldBeVisible =
      this.hasStarted &&
      !this.isPaused &&
      !this.gameState.isInventoryOpen &&
      !isFacingPlayer;
  ...
  getEffectiveRenderDistanceForTier(tier = this.qualityTier) {
    const requested = Math.max(
      2,
      Math.min(6, Number(this.settings?.renderDistance) || 2)
    );
  ...
  updateDynamicQuality(delta) {
    ...
    const fps =
      this.performanceSampler.frames /
      Math.max(0.001, this.performanceSampler.time);
    ...
    this.setStatus(
      `Auto: ${target.toUpperCase()} | ${Math.round(fps)} FPS | res ${((profile?.resolution || 1) * 100).toFixed(0)}% | rd ${this.getEffectiveRenderDistanceForTier(target)}`
    );
  }
  ...
  handlePrimaryAction(delta) {
    ...
    const hasEnemyTarget = this.entities.hasHostileTarget(
    ...
    const mined = this.world.mineBlockProgress(
      this.camera.instance,
      delta,
      selectedItem,
      this.gameState.mode
  ...
  handleSecondaryAction() {
    ...
    const entityInt = this.entities.interactEntityFromCamera(
      this.camera.instance
    );
  ...
  pickBlock() {
    if (!this.hasStarted || this.isPaused) return;
    const hit = this.world.raycastBlocks?.(
      this.camera.instance,
      6,
      false,
      this._camHead,
  ...
  animate(timestamp = 0) {
    ...
    const canSimulate =
      this.hasStarted && !this.isPaused && !this.gameState.isInventoryOpen;
    ...
    const speed = Math.sqrt(
      playerVel.x * playerVel.x + playerVel.z * playerVel.z
    );
    ...
    if (this.hand.lastHeldId !== selectedId) {
      try {
        ...
        console.warn(
          '[ArloCraft] Failed to update held item viewmodel:',
          error
        );
      }
  ...
  updateUnderwaterState() {
    ...
    const submerged = this.world.isPositionInWater(
      camPos.x,
      camPos.y,
      camPos.z
    );
  ...
  async updatePlayerSkin(username) {
    try {
      ...
      console.error('[ArloCraft] Skin Error:', e);
    }
  }
  ...
  animatePlayer(delta) {
    ...
    const speed = Math.sqrt(
      this.physics.velocity.x ** 2 + this.physics.velocity.z ** 2
    );
    if (speed > 0.05) {
  ...
  initDebugOverlay() {
    ...
    overlay.style.cssText =
      'position:fixed;top:96px;left:24px;z-index:100000;background:rgba(0,0,0,0.6);color:#0f0;padding:8px 14px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);pointer-events:none;font-family:monospace;font-size:12px;display:none;';
