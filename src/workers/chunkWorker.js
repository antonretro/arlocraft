    ...
    async generateChunk({
        ...
        for (let lx = 0; lx < chunkSize; lx++) {
            for (let lz = 0; lz < chunkSize; lz++) {
                ...
                const terrainHeight = getColumnHeight(router, workerNoise, wx, wz, currentSeed);
                const waterLevel    = SEA_LEVEL + (biome.waterLevelOffset ?? 0);

                // ── Surface block ──────────────────────────────────────────
                let surfaceId = terrainHeight <= waterLevel ? 'sand' : biome.surfaceBlock;
                const hasRoad    = !inSpawnZone && terrainHeight > waterLevel && isPathAt(wx, wz);
                const hasHighway = !inSpawnZone && terrainHeight > waterLevel && isHighwayAt(wx, wz);
                if (hasHighway) {
                ...
                } else if (terrainHeight > 58 && surfaceId !== 'path_block' && biome.surfaceBlock !== 'snow_block') {
                    // Snow cap override for non-snow biomes above y = 58
                    surfaceId = 'snow_block';
                }

                const surfaceCarved = isCave(workerNoise, wx, terrainHeight, wz, terrainHeight, cavesEnabled);
                ...
                for (let d = 1; d <= 3; d++) {
                    ...
                    if (isCave(workerNoise, wx, y, wz, terrainHeight, cavesEnabled)) continue;
                ...
                if (MIN_TERRAIN_Y >= startY && MIN_TERRAIN_Y < endY) {
                    setPlannedBlock(planMap, changedMap, wx, MIN_TERRAIN_Y, wz, 'bedrock');
                }
                if (MIN_TERRAIN_Y + 1 >= startY && MIN_TERRAIN_Y + 1 < endY && hash2D(wx + 44, wz - 11, currentSeed) > 0.4) {
                    setPlannedBlock(planMap, changedMap, wx, MIN_TERRAIN_Y + 1, wz, 'bedrock');
                }
                if (MIN_TERRAIN_Y + 2 >= startY && MIN_TERRAIN_Y + 2 < endY && hash2D(wx - 22, wz + 88, currentSeed) > 0.8) {
                    setPlannedBlock(planMap, changedMap, wx, MIN_TERRAIN_Y + 2, wz, 'bedrock');
                }
                ...
                if (terrainHeight + 1 >= startY && terrainHeight + 1 < endY) {
                    ...
                    } else if (shouldPlaceAnton(wx, wz, terrainHeight, corruptionEnabled)) {
                        setPlannedBlock(planMap, changedMap, wx, terrainHeight + 1, wz, 'anton');
                    }
                }

                if (!inSpawnZone && terrainHeight > waterLevel && terrainHeight + 1 >= startY && terrainHeight + 1 < endY) {
                    const tntRoll  = hash2D(wx + 777,  wz - 313, currentSeed);
                    const nukeRoll = hash2D(wx - 1441, wz + 918, currentSeed);
                    if (tntRoll  > 0.9989)  setPlannedBlock(planMap, changedMap, wx, terrainHeight + 1, wz, 'tnt');
                    if (nukeRoll > 0.99972) setPlannedBlock(planMap, changedMap, wx, terrainHeight + 1, wz, 'nuke');
                }
                ...
                if (!surfaceCarved && !inSpawnZone && !isHighAltitude &&
                        shouldPlaceTree(workerNoise, wx, wz, terrainHeight, biome, currentSeed)) {
                    ...
                    if (checkY >= startY && checkY < endY) {
                        addTree(planMap, changedMap, wx, checkY, wz, biome, corruptionEnabled,
                                workerNoise, currentSeed, setPlannedBlock);
                    }
                }
                if (!surfaceCarved && !inSpawnZone && !isHighAltitude && terrainHeight > waterLevel) {
                    if (terrainHeight >= startY && terrainHeight < endY) {
                        addGroundLife(planMap, changedMap, wx, terrainHeight, wz, biome,
                                    workerNoise, currentSeed, setPlannedPlant, surfaceId);
                    }
                }
            }
        }
        ...
        if (cavesEnabled) {
            ...
            for (let lx = 0; lx < chunkSize; lx++) {
                for (let lz = 0; lz < chunkSize; lz++) {
                    ...
                    for (let wy = Math.min(terrainH - 4, endY - 1); wy >= Math.max(MIN_TERRAIN_Y + 1, startY); wy--) {
                        if (!isCave(workerNoise, wx, wy, wz, terrainH, true)) continue;

                        const biome = getCaveBiome(workerNoise, wx, wy, wz, terrainH, currentSeed);
                        ...
                        if (floorId !== undefined) {
                            ...
                            switch (biome) {
                                case 'dripstone':
                                    ...
                                    if (floorId === 'stone') {
                                        setPlannedBlock(planMap, changedMap, wx, floorY, wz, 'dripstone');
                                    }
                                    if (roll < 0.12 && isAir(wx, wy, wz) && isAir(wx, wy + 1, wz)) {
                                        setPlannedBlock(planMap, changedMap, wx, wy, wz, 'dripstone');
                                    }
                                    break;
                                case 'lush':
                                    if (floorId === 'stone' || floorId === 'dirt') {
                                        if (roll < 0.55) setPlannedBlock(planMap, changedMap, wx, floorY, wz, 'moss_block');
                                        else             setPlannedBlock(planMap, changedMap, wx, floorY, wz, 'clay');
                                    }
                                    break;
                                case 'deep_dark':
                                    if (floorId === 'stone') {
                                        setPlannedBlock(planMap, changedMap, wx, floorY, wz, 'deepslate');
                                    }
                                    break;
                                case 'ice':
                                    if (floorId === 'stone' || floorId === 'dirt') {
                                ...
                                case 'mushroom':
                                    if (floorId === 'stone' || floorId === 'dirt') {
                                        setPlannedBlock(planMap, changedMap, wx, floorY, wz, 'mycelium');
                                    }
                                    // Mushroom plants on top of solid floor
                                    if (roll < 0.09 && isAir(wx, wy, wz)) {
                                        const shroomId = roll < 0.045 ? 'mushroom_brown' : 'mushroom_red';
                            ...
                            switch (biome) {
                                ...
                                case 'lush':
                                    ...
                                    if (roll > 0.82 && isAir(wx, wy, wz)) {
                                        const vineId = roll > 0.94 ? 'cave_vines_lit' : 'cave_vines';
                                ...
                                case 'ice':
                                    ...
                                    if (roll > 0.85 && isAir(wx, wy, wz)) {
                                        setPlannedBlock(planMap, changedMap, wx, wy, wz, 'packed_ice');
                                    }
                                    if (ceilId === 'stone') {
                                        setPlannedBlock(planMap, changedMap, wx, ceilY, wz, 'packed_ice');
                                    }
                                    break;
                                default:
                                    break;
                        ...
                        for (const [nx, ny, nz] of neighbors) {
                            ...
                            const wallRoll = hash2D(nx + ny * 5, nz - ny * 11, currentSeed + 4817);
                            switch (biome) {
                                case 'dripstone':
                                    if (wallRoll < 0.25) {
                                        setPlannedBlock(planMap, changedMap, nx, ny, nz,
                                ...
                                case 'deep_dark':
                                    ...
                                    if (wallRoll < 0.04) {
                                        setPlannedBlock(planMap, changedMap, nx, ny, nz, 'obsidian');
                                    }
                                    break;
                                case 'ice':
                                    if (wallRoll < 0.5) {
                                        setPlannedBlock(planMap, changedMap, nx, ny, nz, 'packed_ice');
                                    }
                                    break;
                                case 'mushroom':
                                    if (wallRoll < 0.08) {
                                        setPlannedBlock(planMap, changedMap, nx, ny, nz, 'glowstone');
                                    }
                                    break;
                                case 'lush':
                                    if (wallRoll < 0.15) {
                                        setPlannedBlock(planMap, changedMap, nx, ny, nz, 'moss_block');
                                    }
                                    break;
                                default:
                                    break;
        ...
        for (const [key, id] of changedMap.entries()) {
            ...
            if (chunkCoord(x, chunkSize) !== cx || chunkCoord(y, chunkSize) !== cy || chunkCoord(z, chunkSize) !== cz) continue;
            if (id === null) {
                planMap.delete(key);
            } else {
                planMap.set(key, id);
