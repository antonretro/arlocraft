...
import {
  blockIdToDisplayName,
  normalizeBlockVariantId,
} from '../data/blockIds.js';
...
export class InventoryUI {
  constructor(gameState, options = {}) {
    ...
    this.getCraftingRecipeName =
      options.getCraftingRecipeName ?? (() => 'Unknown');
  ...
  cacheRefs() {
    ...
    this.refs.selectedItemStats = document.getElementById(
      'selected-item-stats'
    );
  ...
  init() {
    ...
    window.addEventListener('inventory-toggle', (event) =>
      this.toggleInventoryUI(Boolean(event.detail))
    );
  ...
  createSlot(idx, type) {
    ...
    slot.addEventListener('pointerdown', (event) =>
      this.onSlotPointerDown(idx, type, event)
    );
    return slot;
  }
  ...
  onGlobalPointerUp(event) {
    ...
    if (!Number.isFinite(idx) || (type !== 'inventory' && type !== 'crafting'))
      return;

    const changed = this.applyCarryToSlot(
      idx,
      type,
      event.button === 2 ? 2 : this.lastPointerButton
    );
    this.pointerPickupPending = false;
    this.pointerPickupMoved = false;
    if (!changed) return;
  ...
  applyCarryToSlot(idx, type, button = 0) {
    ...
    if (button === 2) {
      if (!slotItem) {
        ...
        this.sameItem(slotItem, this.carryItem) &&
        (slotItem.count ?? 1) < MAX_STACK
      ) {
  ...
  returnCarryToInventory() {
    ...
    const sameStack = inv.find(
      (slot) => this.sameItem(slot, this.carryItem) && (slot.count ?? 1) < 99
    );
    if (sameStack) {
    ...
    if (this.carryItem) {
      const craftingEmpty = this.gameState.craftingGrid.findIndex(
        (slot) => !slot
      );
      if (craftingEmpty >= 0) {
  ...
  renderSelectedItem() {
    ...
    if (!item) {
      ...
      const offhandText = offhand
        ? `Offhand ${offhand.id.toUpperCase()} x${offhand.count ?? 1}`
        : 'Offhand EMPTY';
    ...
    if (block) {
      ...
      const offhand = this.gameState.getOffhandItem();
      const offhandText = offhand
        ? `${offhand.id.toUpperCase()} x${offhand.count ?? 1}`
        : 'EMPTY';
  ...
  sortInventory() {
    ...
    for (const item of items) {
      const existing = merged.find(
        (m) => m.id === item.id && m.kind === item.kind && m.count < 99
      );
      if (existing) {
  ...
  updateHotbarSelection(slot) {
    ...
    slots.forEach((node, index) =>
      node.classList.toggle('active', index === slot)
    );
  }
}
