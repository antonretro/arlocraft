...
export class IconService {
    ...
    getBlockTextureSet(id) {
        const legacyMap = {
            ...
            'gravel': 'gravel', 'glass': 'glass', 'snow': 'snow', 'ice': 'ice',
            'obsidian': 'obsidian', 'bedrock': 'bedrock', 'water': 'water_still',
            'lava': 'lava_still', 'sand': 'sand', 'cobblestone': 'cobblestone',
            'oak_plank': 'oak_planks', 'sandstone': 'sandstone', 'dirt': 'dirt',
            'stone': 'stone', 'tnt': 'tnt', 'crafting_table': 'crafting_table',
            'lantern': 'lantern', 'path_block': 'dirt_path', 'clay': 'clay', 'brick': 'bricks'
        };
    ...
    getIconPath(itemId) {
        ...
        const toolMap = {
            pick_wood: 'wooden_pickaxe', sledge_iron: 'iron_pickaxe', data_drill: 'netherite_pickaxe',
            sword_wood: 'wooden_sword', power_blade: 'iron_sword', glitch_saber: 'diamond_sword',
            axe_wood: 'wooden_axe', byte_axe: 'diamond_axe', echo_dagger: 'stone_sword',
            arc_spear: 'trident', plasma_hammer: 'iron_axe', static_bow: 'bow',
            pulse_pistol: 'crossbow', rail_rifle: 'crossbow', scatter_blaster: 'crossbow',
            decoder_wand: 'stick', magnet_glove: 'iron_ingot', rocket_boots: 'iron_boots',
            grappler: 'fishing_rod', scanner: 'compass_16', master_key: 'gold_ingot',
            apple: 'apple', bread: 'bread', steak: 'cooked_beef', carrot: 'carrot',
            potato: 'baked_potato', corn: 'wheat', blueberry: 'sweet_berries', strawberry: 'sweet_berries',
            melon_slice: 'melon_slice', pumpkin_pie: 'pumpkin_pie', cookie: 'cookie',
            cooked_fish: 'cooked_cod', honey_bottle: 'honey_bottle', mushroom_brown: 'brown_mushroom',
...
<text x="32" y="39" text-anchor="middle" font-family="Segoe UI, Arial, sans-serif" font-size="23" font-weight="700" fill="url(#accent)">${monogram}</text>
    ...
    _buildAbbrevMap() {
        ...
        const candidate = (id) => {
            ...
            const two = parts.length >= 2 ? initials.slice(0, 2).join('') : parts[0].slice(0, 2).toUpperCase();
            if (!used.has(two)) return two;
            const three = parts.length >= 3 ? initials.slice(0, 3).join('') : (parts.length === 2 ? (initials[0] + parts[1].slice(0, 2).toUpperCase()) : parts[0].slice(0, 3).toUpperCase());
    ...
    getIconAccentColor(itemId) {
        ...
        const palette = {
            stone: '#7bc8ff', dirt: '#f0a56d', grass: '#76f59f', wood: '#ffc176',
            leaves: '#8de86b', sand: '#ffe384', water: '#7bd2ff', iron: '#b8c8ff',
            gold: '#ffe36a', diamond: '#6bffe6', static_bow: '#8ad7ff', power_blade: '#ff8fb2',
            pick_wood: '#ffcf8d', magnet_glove: '#ffd28f', virus: '#d08bff', anton: '#ffb0d7'
        };
