export class HUDCore {
    ...
    init() {
        ...
        window.addEventListener('mode-changed', (event) => this.updateMode(event.detail));
    ...
    updateCoordinates(position, yaw = 0, world = null) {
        ...
        if (this.refs.biomePill) this.refs.biomePill.textContent = biomeLabel.toUpperCase();
        if (this.refs.minimapContext) this.refs.minimapContext.textContent = `Seed ${seed} | ${biomeLabel}`;
    ...
    updateMode(mode) {
        if (this.refs.modeIndicator) this.refs.modeIndicator.textContent = `${mode} MODE`;
        if (this.refs.modePill) this.refs.modePill.textContent = mode;
    }
