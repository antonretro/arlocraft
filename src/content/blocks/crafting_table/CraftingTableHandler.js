...
function getOverlay() { return document.getElementById('crafting-table-overlay'); }
...
function close() {
    ...
    if (_game) {
        for (let i = 0; i < 9; i++) {
            if (_grid[i]) {
                _game.gameState.addItemToInventory(_grid[i].id, _grid[i].count, _grid[i].kind ?? 'block');
                _grid[i] = null;
            }
        }
        if (_carryItem) {
            _game.gameState.addItemToInventory(_carryItem.id, _carryItem.count, _carryItem.kind ?? 'block');
            _carryItem = null;
        }
...
function renderInventory() {
    ...
    mainGrid.innerHTML = '';
    for (let i = 9; i < 36; i++) {
        mainGrid.appendChild(makeSlot('inventory', i, _game.gameState.inventory[i]));
    }
    hotbarGrid.innerHTML = '';
    for (let i = 0; i < 9; i++) {
        hotbarGrid.appendChild(makeSlot('inventory', i, _game.gameState.inventory[i]));
    }
}
    ...
    if (!_carryItem) {
        ...
        if (!slot) {
            if (event.button === 2) {
                ...
                _carryItem.count -= 1;
                if (_carryItem.count <= 0) { _carryItem = null; _carryOrigin = null; }
            } else {
            ...
            const put = event.button === 2 ? 1 : Math.min(space, _carryItem.count ?? 1);
...
function handleResultClick() {
    ...
    const added = _game.gameState.addItemToInventory(result.id, result.count ?? 1, kind);
    if (!added) return;
    for (let i = 0; i < 9; i++) {
        if (!_grid[i]) continue;
...
document.addEventListener('pointerup', (e) => {
    if (!getOverlay()?.style.display || getOverlay().style.display === 'none') return;
    if (!_carryItem) return;
    const target = document.elementFromPoint(e.clientX, e.clientY)?.closest('.bh-slot[data-bh-source]');
