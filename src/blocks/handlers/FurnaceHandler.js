...
const SMELT_RECIPES = [
  ...
  {
    input: 'copper_ore',
    output: { id: 'copper_ingot', count: 1, kind: 'block' },
  },
...
function getOverlay() {
  return document.getElementById('furnace-overlay');
}
  ...
  if (recipe && fs.isActive && fs.input) {
    ...
    if (fs.smeltProgress >= SMELT_MS) {
      ...
      if (!fs.output) {
        ...
        fs.output.id === recipe.output.id &&
        (fs.output.count ?? 1) < 99
      ) {
...
function renderAll() {
  ...
  if (bar) {
    ...
    const pct =
      recipe && fs.isActive
        ? Math.min(100, (fs.smeltProgress / SMELT_MS) * 100)
        : 0;
    bar.style.width = `${pct}%`;
    bar.parentElement?.setAttribute(
      'title',
      recipe ? `Smelting: ${Math.round(pct)}%` : 'No recipe'
    );
  }
  ...
  if (!selectedItem) {
    ...
    if (furnaceItem) {
      _game.gameState.addItemToInventory(
        furnaceItem.id,
        furnaceItem.count ?? 1,
        furnaceItem.kind ?? 'block'
      );
      if (role === 'input') fs.input = null;
      else fs.fuel = null;
      window.dispatchEvent(new CustomEvent('inventory-changed'));
    }
    renderAll();
