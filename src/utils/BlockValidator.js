...
const GROUND_LIFE_IDS = [
    'short_grass', 'tall_grass_bottom', 'tall_grass_top',
...
const TREE_IDS = [
    ...
    'mangrove_log', 'mangrove_leaves', 'acacia_log', 'acacia_leaves',
    'dark_oak_log', 'dark_oak_leaves', 'cherry_log', 'cherry_leaves',
];
...
const TERRAIN_IDS = [
    ...
    'redstone_ore', 'copper', 'deepslate_diamond_ore', 'deepslate_redstone_ore',
    'deepslate_gold_ore', 'deepslate_lapis_ore', 'deepslate_iron_ore',
...
function validatePairLinks(pairWarnings) {
    for (const block of BLOCKS) {
        ...
        if (Boolean(pairId) !== Boolean(pairOffsetY)) {
            pairWarnings.push(`[pairs] "${block.id}" must define both pairId and pairOffsetY`);
            continue;
        }
        ...
        if (!pairBlock) {
            pairWarnings.push(`[pairs] "${block.id}" points to missing pair "${pairId}"`);
            continue;
        }

        const reversePairId = typeof pairBlock.pairId === 'string' && pairBlock.pairId.trim()
        ...
        if (block.renderType === 'paired_plant' && !block.deco) {
            pairWarnings.push(`[pairs] "${block.id}" is paired_plant but missing deco:true`);
        }
    }
}
